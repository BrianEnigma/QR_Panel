...
{
    static int s_previousProgressCount = 0;
    unsigned int ledProgressCount = LED_COUNT / 2;
    float percent = (float) step / (float) steps;
    ledProgressCount = (int)((float) ledProgressCount * percent);
    ...
    if (s_previousProgressCount > ledProgressCount)
        s_previousProgressCount = 0;
    // Instantly fill in previous count.  In theory, it should already be filled in.
    strip.clear();
    for (int i = 0; i < s_previousProgressCount; i++)
    {
        strip.setPixelColor(i, strip.Color(0x00, 0xff, 0x00));
        strip.setPixelColor(LED_COUNT - i - 1, strip.Color(0x00, 0xff, 0x00));
    }
    strip.show();
    // Fill in from previous to current, only a little slow like an animation.
    for (int i = s_previousProgressCount; i < ledProgressCount; i++)
    {
        delay(40);
        strip.setPixelColor(i, strip.Color(0x00, 0xff, 0x00));
        strip.setPixelColor(LED_COUNT - i - 1, strip.Color(0x00, 0xff, 0x00));
    ...
    s_previousProgressCount = ledProgressCount;
}
